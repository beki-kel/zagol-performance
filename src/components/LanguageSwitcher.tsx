          ...
          <button
            ...
            className={`relative px-2 transition-colors duration-300 ${
              availableLocale === currentLocale
                ? "text-[#025f96] after:scale-100"
                : "text-neutral-800 hover:text-blue-400 after:scale-0"
            }`}
          >
            {availableLocale.toUpperCase()}
            <span className="absolute bottom-[-2px] left-0 w-full h-[2px] bg-blue-600 origin-center scale-0 after:transition-transform duration-300 ease-in-out"></span>
          </button>
          {index < locales.length - 1 && (
            <span
              className="text-neutral-900 font-light animate-fade-in-out duration-700"
              aria-hidden="true"
            >
              /
            </span>
